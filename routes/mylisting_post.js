...
router.post("/delete/:items_id", (req, res) => {
  ...
  delItemFnc.delItem(items_id).then((items) => {
...
router.get("/update/:itemid/:userid", (req, res) => {
  ...
  getItemFnc
    .getItembyUserIDItemId(user_id, item_id)
...
router.post("/sold/:itemid", (req, res) => {
  const item_id = req.params.itemid;
  // console.log("inside post, item_id", item_id);
  itemQuery.markAsSold(item_id).then((response) => {
    res.redirect("/items/admin");
  });
});
